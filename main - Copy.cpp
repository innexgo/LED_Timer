...
void initTimer(void) {
  ...
  led_per_half_sec = ((60/duration_time)/2);
  timer_on = true;
  ...
  color_file.close();
  int color = std::strtol(color_chars, 0, 16);
...
void tickTimer(void) {
  if ((corrected_millis - previous_timer_millis) >= 500) {
    ...
    if ((int)((timer_count * led_per_half_sec) + 0.001) > dark_LED_count) {
